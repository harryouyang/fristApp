 ...
 */
public class PGFileUtil {

    public static final String TAG = PGFileUtil.class.getName();
    ...
    public static File getCacheDir(String dirName) {
        File result;
        if (existsSdcard()) {
            File cacheDir = PGX.app().getExternalCacheDir();
            if (cacheDir == null) {
                result = new File(Environment.getExternalStorageDirectory(),
                        ".android/" + PGX.app().getPackageName() + "/" + dirName);
            } else {
                result = new File(cacheDir, dirName);
            }
        } else {
            result = new File(PGX.app().getFilesDir(), dirName);
        }
    ...
    public static boolean copy(String fromPath, String toPath) {
        ...
        PGIOUtil.deleteFileOrDir(toFile);
        ...
        if (toDir.exists() || toDir.mkdirs()) {
            ...
            try {
                ...
                PGIOUtil.copy(in, out);
                result = true;
            } catch (Throwable ex) {
                PGLogUtil.d(TAG, ex.getMessage());
                result = false;
            } finally {
                PGIOUtil.closeQuietly(in);
                PGIOUtil.closeQuietly(out);
            }
        }
        return result;
    }
